  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mx-auto"></div>
          <p className="mt-6 text-slate-600 font-medium">Loading advisor dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="bg-white py-4 rounded-2xl shadow-xl border border-slate-200 max-w-md text-center">
          ...
          <h1 className="text-xl font-semibold text-slate-800 mb-2">Advisor Profile Not Found</h1>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  <h1 className="text-2xl font-bold text-slate-800">{advisor.name}</h1>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${
                    advisor.busy 
                      ? 'bg-red-100 text-red-700' 
                      : 'bg-green-100 text-green-700'
                  }`}>
                    <span className={`w-2 h-2 rounded-full ${advisor.busy ? 'bg-red-500' : 'bg-green-500 animate-pulse'}`}></span>
          ...
          <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-slate-800">Chat Requests</h2>
                  <p className="text-sm text-slate-500">
                    {chatRequests.length} active {chatRequests.length === 1 ? 'session' : 'sessions'}
                  </p>
                </div>
              </div>
          ...
          <div className="p-6">
            {chatRequests.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-700 font-medium text-lg">No active requests</p>
                <p className="text-slate-500 text-sm mt-1 max-w-sm mx-auto">
                  New chat requests will appear here automatically when users reach out for help.
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-4 mt-1.5 text-sm text-slate-500 flex-wrap">
                            {(request.user?.phone || request.user?.phoneNumber) && (
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 text-slate-600 rounded-md text-xs">
                              ...
                              {request.payment?.method === 'wallet' || request.paymentMethod === 'wallet' 
                                ? 'Wallet' 
                                : 'Razorpay'}
                            </span>
                          </div>
